...
object P17 {
  ...
  def splitFun[A](n: Int, ls: List[A]): (List[A], List[A]) = 
    (ls.take(n), ls.drop(n))
  
  def splitTail[A](n: Int, ls: List[A]): (List[A], List[A]) = {
...
println(P17.splitFun(3, List('a, 'b, 'c, 'd, 'e, 'f, 'g, 'h, 'i, 'j, 'k)))
println(P17.splitTail(3, List('a, 'b, 'c, 'd, 'e, 'f, 'g, 'h, 'i, 'j, 'k)))